...
class Game:
	...
	def addSparkle(self,objects,event):
		x,y = event.pos
		sz = 20
		objects.append(Touch(event.pos,randint(5,sz>>1),0,randint(0,6)*0.13,Colour(Colour.paletteFlame)))
	...
	def gameLoop(self):
		...
		while keepGoing:
			...
			for event in pygame.event.get():
				if event.type == pygame.QUIT:
					...
					if event.button == 1: # 1 == Left
						...
						print event
						objects.append(Touch(event.pos,80,randint(-2,2),1,Colour()))
				elif event.type == MOUSEMOTION:
					if random() <= 0.8:
						objects = self.addSparkle(objects,event)
				else:
					print event
			...
			#print timeDelta
			if timeDelta > 1000: print "System is running at or below 1FPS."
